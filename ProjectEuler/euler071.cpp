...
// checks if a/b < c/d:
// if all numbers positive: a*d < c*b
// BigInt a.compare(b) results: 0 if a == b, -1 if a < b, 1 if a > b
bool checkLess(BigInt a, BigInt b, BigInt c, BigInt d) {
    BigInt x = a * d;
    BigInt y = c * b;
    if(x.compare(y) == -1) 
        return true;
...
/*
if two neighboring fractions a/b < c/d with bc-ad = 1 
are neighbors in a farey sequence then the first term that appears between them
as the order of the farey sequence is incremented is:
    (a+c) / (b+d)
    
0/1 < 1/1 fulfill these properties -> start with a=0 b=c=d=1 and increment 
*/
int main() {
    ios_base::sync_with_stdio(false); cin.tie(NULL); cout.tie(NULL);
    int t;
    unsigned long long int a_in,b_in,N;
    cin >> t;
    while(t--) {
        cin >> a_in >> b_in >> N;
        const BigInt n (N);
        const BigInt a_in_big (a_in);
        const BigInt b_in_big (b_in);
        
        BigInt a = 0;
        BigInt b = 1;
        BigInt c = 1;
        BigInt d = 1;
        BigInt new_numerator, new_denominator;
        
        while((a+c).compare(n) <= 0) {
            new_numerator = a + c;
            new_denominator = b + d;

            
            if(checkLess(new_numerator, new_denominator, a_in_big, b_in_big) ) { 
                a = new_numerator;
                b = new_denominator;
            }
            else {
        ...
        if(n.compare(b+d) >= 0) {
            unsigned long long aa = stoull(a.toString());
            unsigned long long bb = stoull(b.toString());
            unsigned long long cc = stoull(c.toString());
            unsigned long long dd = stoull(d.toString());

            unsigned long long diff = N - (bb + dd);
            unsigned long long factor = 1 + diff / dd;
