...
long compute_time(vector<long> machines, long goal, long min_days, long max_days) {
    if(machines.size()==1) return max_days;

    if(min_days==max_days) return min_days;

    long mid = (min_days+max_days) / 2;
    long produced = 0;

    for(int i=0; i<machines.size(); i++)
        produced += floor( mid/machines[i] );

    if(produced >= goal)
        return compute_time(machines,goal,min_days,mid);
    else
        return compute_time(machines,goal,mid+1,max_days);
}

long minTime(vector<long> machines, long goal) {
    sort(machines.begin(), machines.end() );
    
    return compute_time(machines, goal, machines.front()*goal/machines.size(), machines.front()*goal);
}
